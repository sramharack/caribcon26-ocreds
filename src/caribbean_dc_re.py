...
# ═══════════════════════════════════════════════════════
def case_a(gas_mc=55., solar_capex=1100., co2_price=0.):
    """gas_mc: gas fuel+VOM $/MWh; co2_price: $/tCO2 (adds EF*price to marginal cost)."""
    r=.07; n=pypsa.Network(); n.set_snapshots(SNAP)
    n.add("Bus","TT",carrier="AC",x=-61.3,y=10.5)
    # NGC gas fleet ~1800MW
    n.add("Generator","TT_gas",bus="TT",carrier="gas",p_nom=1800,marginal_cost=gas_mc+EF['gas']*co2_price,p_min_pu=.25)
    ...
    n.add("Generator","TT_sol1",bus="TT",carrier="solar",p_nom_extendable=True,p_nom_max=300,
          capital_cost=acap(solar_capex,12,25,r),marginal_cost=.5,p_max_pu=solar_p(.18,10.5,11))
...
def congestion(n,thr=.99):
    """Hours each transmission link runs at >= thr of its rating."""
    out={}
    for l in n.links.index:
        if l.startswith('tx_'):
            out[l]=int((n.links_t.p0[l]>=thr*n.links.loc[l,'p_nom']).sum())
    return out

def storage_caps(n):
    return {s:(float(n.stores.loc[s,'e_nom_opt']),float(n.stores.loc[s,'e_nom_max'])) for s in n.stores.index}

# ── Post-hoc DRO ──
# Hurricane category profiles (p_c, g_c, h_c): share of landfalling storms by
# Saffir-Simpson category, fractional grid outage, outage duration (h).
# Stylized values -- see docs/dro_parameters.md for basis and limitations.
DRO_CATS={'C1':(.35,.15,24),'C2':(.25,.30,48),'C3':(.20,.50,72),'C4':(.15,.70,120),'C5':(.05,.90,240)}
# island: (strike-exposure multiplier s_i, BTM backup energy MWh, DC IT MW)
# BTM MWh = modelled BTM e_nom_max (TT: 5*1.5*4=30, JM: 15*1.4*4=84)
DRO_ISLANDS={'Trinidad':(.3,30,5),'Jamaica':(.8,84,15)}
def dro_calc(cats=DRO_CATS,islands=DRO_ISLANDS,eps_max=.20,n_eps=7):
    """Stylized worst-case tilt: probability mass shifted toward high-outage
    categories in proportion to eps (not an exact Wasserstein DRO solve)."""
    eps_v=np.linspace(0,eps_max,n_eps); res={}
...
if __name__=='__main__':
    ...
    rj={'CaseA':ra,'CaseB':rb,'dro':dro,'jm_congestion_hours':congestion(nb),'jm_storage_opt_vs_max':storage_caps(nb)}
    with open(f'{RDIR}/results.json','w') as f: json.dump(rj,f,indent=2,default=str)
